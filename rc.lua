...
-- {{{ Mem Usage
  -- Icon
    ...
    ramicon:set_resize(false)
  -- Percentage
    mempct = wibox.widget.textbox()
    mempct:set_align('right')
...
-- {{{ Filesystem info
-- Home Icons
fshomeicon = wibox.widget.imagebox()
fshomeicon:set_image(beautiful.widget_fshome)
fshomeicon:set_resize(false)
-- Root icon
fsrooticon = wibox.widget.imagebox()
fsrooticon:set_image(beautiful.widget_fsroot)
fsrooticon:set_resize(false)
-- Remaining
...
-- Readout
  ...
  -- Percentage
    cpu1 = wibox.widget.textbox()
    vicious.register(cpu1, vicious.widgets.cpu, "(1): $1%")
    cpu2 = wibox.widget.textbox()
...
-- wibox inferior
    ...
    local  capa = wibox.layout.fixed.horizontal()
     capa:add(space)
     capa:add(sysicon)
     capa:add(space)
     capa:add(sys)
